...
function makeBurger(quantity) {
  const burger = 120;
  if (quantity > 5) {
//     console.log("only 5 burger allow to purchase")
    return "sorry max 5 burger allowed";
  }else{
      let totalPrice = burger*quantity;
      return totalPrice;
}
}
 let output = makeBurger(6);
 console.log(output);


 function calculateBusFare(quantity,isBusiness){
      const economy = 550;
      const business = 800;
      if (isBusiness === true){
            let total = quantity * business;
            return total;
      }else{
            let total = quantity*economy;
            return total;
      }
 }

 let test1 = calculateBusFare(4 , true);
 console.log("test 1 cost :", test1);
 let test2 = calculateBusFare(2 , false);
 console.log("test 2 total", test2);
